...
int main()
{
    ...
    VARP x = _RandomNormal({1, 4, 48, 48}, halide_type_of<float>(), 0.0f, 1.0f, 404, 06);
    VARP x1 = _RandomNormal({1, 4, 48, 48}, halide_type_of<float>(), 0.0f, 1.0f, 40, 406);

    VARP output = model.sample(x, context, 7.5f, 5, 0.0f);
    VARP output1 = model.sample(x1, context, 7.5f, 5, 0.0f);
...
*/
    ...
    std::cout << std::endl;
    if (stbi_write_png("/storage/emulated/0/result_0001.png", rdim[2], rdim[1], rdim[3], rptr1, stride_bytes) == 0) throw std::runtime_error("保存失败");
