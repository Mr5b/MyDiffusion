...
int main()
{
    ...
    //executor->setGlobalExecutorConfig(MNN_FORWARD_VULKAN, backendConfig, 1);
    //executor->setGlobalExecutorConfig(MNN_FORWARD_CPU, backendConfig, 8);
    ...
    //executor->setGlobalExecutorConfig(MNN_FORWARD_USER_0, backendConfig, 1);
    //executor->setGlobalExecutorConfig(MNN_FORWARD_AUTO, backendConfig, 8);
    ...
    VARP x = _RandomNormal({1, 4, 8, 8}, halide_type_of<float>(), 0.0f, 1.0f, 404, 06);
    VARP x1 = _RandomNormal({1, 4, 8, 8}, halide_type_of<float>(), 0.0f, 1.0f, 406, 406);
    
    auto start = std::chrono::high_resolution_clock::now();
    VARP output = model.sample_low(x, context, 0.0f, 5, 0.0f);
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> time0 = end - start;
    std::cout << "😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤Sample time0: " << time0.count() << "s" << std::endl;
    
    start = std::chrono::high_resolution_clock::now();
    VARP output1 = model.sample_low(x1, context, 0.0f, 5, 0.0f);
    end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> time1 = end - start;
    std::cout << "😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤Sample time1: " << time1.count() << "s" << std::endl;
    
    //output.fix(VARP::CONSTANT);
    
    
    //output1.fix(VARP::CONSTANT);
    ...
    output1 = model.first_stage_model_->decode(output1);

    start = std::chrono::high_resolution_clock::now();
    output.fix(VARP::CONSTANT);
    end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> time2 = end - start;
    std::cout << "😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤Decode time0: " << time2.count() << "s" << std::endl;
    ...
    std::cout << "😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤Decode time1: " << time3.count() << "s" << std::endl;

    output1 = _Permute(output1, {0, 2, 3, 1});
    output1 = (output1 + _Scalar<float>(1)) * _Scalar<float>(127.5);
    output1 = _Minimum(_Maximum(output1, _Scalar<float>(0)), _Scalar<float>(255));
    output1 = _Cast<uint8_t>(output1);
    //pppp(output);
    


    auto rptr1 = output1->readMap<uint8_t>();
    
    //std::cout << "😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤TIME0:" << time.count() << "s" << std::endl;

    for (int i = 0, n = output1->getTensor()->elementSize(); i < 10; i++)
    {
        std::cout << int(rptr1[i]) << " ";
    }
    std::cout << "..." << std::endl;
/*    
    std::cout << "😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤Sample time0: " << time0.count() << "s" << std::endl;
    std::cout << "😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤Sample time1: " << time1.count() << "s" << std::endl;
    std::cout << "😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤😤Sample time2: " << time2.count() << "s" << std::endl;
*/

    
    const INTS& rdim = output1->getInfo()->dim;
    
    int stride_bytes = rdim[2]*rdim[3]*sizeof(uint8_t);
    for (auto d : rdim) std::cout << d << " ";
    std::cout << std::endl;
    if (stbi_write_png("/storage/emulated/0/result_0005.png", rdim[2], rdim[1], rdim[3], rptr1, stride_bytes) == 0) throw std::runtime_error("保存失败");
